...
def select_and_rewrite_psy_with_gemini(news_list, category_name):
    ...
    prompt = (
        ...
        f"{candidates_text}\n"
        "Інструкція з написання:\n"
        "1. Обери ОДНОГО найкращого кандидата, який найкраще підходить для вказаної категорії.\n"
        ...
        "4. Пост має бути лаконічним і СТРОГО до 650-700 символів (разом із пробілами).\n"
...
async def post_psy_category_update(client, category_name):
    ...
    try:
        ...
        if not photo_path or not os.path.exists(photo_path):
            if os.path.exists("psy_default.png"):
                photo_path = "psy_default.png"
                print("🎨 Використовую дежурний малюнок psy_default.png за замовчуванням!")
