...
async def select_and_rewrite_news_with_gemini(news_items):
    ...
    prompt = (
        ...
        f"{candidates_text}\n"
        "СУВОРІ ІНСТРУКЦІЇ:\n"
        "1. Обери ОДНУ найважливішу та найцікавішу новину.\n"
        "2. Напиши професійний, захоплюючий рерайт цієї новини українською мовою.\n"
        "3. Пост має бути лаконічним, чітким і строго до 600-650 символів (разом із пробілами).\n"
        "4. КРИТИЧНО: Пиши бездоганною, природною українською мовою без русизмів чи кальок.\n"
        "5. КРИТИЧНО: Якщо в тексті згадуються теми штучного інтелекту, зроби посилання на партнерський канал про ШІ: @te_shoo_treba ('дізнатися більше можна в <b><a href=\"https://t.me/te_shoo_treba\">Те що треба | AI</a></b>'). "
        "Якщо згадується психологія чи стрес, зроби відсилання на партнерський канал про психологію: @ncux_olo_guY ('підтримати ментальне здоров\\'я допоможе <b><a href=\"https://t.me/ncux_olo_guY\">Нейро-Апгрейд</a></b>').\n"
        "6. Використовуй ТІЛЬКИ HTML-теги для виділення жирного тексту: <b>текст</b>. НІКОЛИ не використовуй маркдаун зі зірочками (типу ** чи *).\n"
        "7. СУВОРЕ ПРАВИЛО: НЕ ПИШИ ЖОДНИХ ВСТУПНИХ СЛІВ, МІРКУВАНЬ, РЕПЛІК ШІ ЧИ ПРИВІТАНЬ ('Давайте проаналізуємо', 'Я проаналізував', 'Ось ідея'). Пиши ОДРАЗУ чистий текст поста.\n"
        "8. Переконайся, що пост ПОВНІСТЮ дописаний і всі речення закінчені.\n"
        ...
        "Почни відповідь з 'INDEX: X' та пиши виключно українською мовою з використанням HTML-форматування <b>...</b>."
    )
...
async def select_and_compile_with_gemini(news_list, my_last_posts, category_name, channel_type, prefer_groq=False):
    ...
    prompt = (
        ...
        f"Твоє завдання — написати один унікальний готовий до публікації пост, спираючись на свіжі новини.\n\n"
        ...
        f"{news_pool_text}\n\n"
        "Інструкція:\n"
        "1. Обери найцікавішу унікальну новину, якої немає в списку 15 останніх постів.\n"
        f"2. Напиши повноцінний, цікавий пост {target_lang}.\n"
        "3. Об'єм поста: 800-1200 символів. Речення мають бути повністю закінченими (без багатокрапок у кінці).\n"
        "4. КРИТИЧНЕ ПРАВИЛО: У блоці POST_TEXT пиши ТІЛЬКИ текст поста для підписників! Жодних міркувань, привітань чи коментарів ШІ (категорично заборонено писати 'Розглянувши останні публікації...', 'Я пропоную нову тему...', 'Ось пост:' тощо). Починай відразу з заголовку поста!\n"
        "5. ФОРМАТУВАННЯ: Заборонено використовувати маркдаун із зірочками (** чи *). Для виділення жирного тексту використовуй ТІЛЬКИ HTML-теги: <b>текст</b>.\n"
        "6. КРИТИЧНО: Заборонено додавати посилання на джерела або слово 'Джерела'.\n"
        f"{extra_rules}\n"
        "7. Обов'язково додай опис сцени для генератора зображень англійською мовою (IMAGE_PROMPT). Вона має бути суто візуальною, без тексту.\n\n"
        "Суворий формат відповіді:\n"
        "RESPONSE_STATUS: UNIQUE\n"
        "POST_TEXT:\n"
        "<твій текст поста тут>\n"
        "IMAGE_PROMPT:\n"
        "<опис сцени для генератора зображень англійською мовою без тексту та логотипів>\n\n"
        "Якщо в пулі новин немає придатних тем, згенеруй корисний пост-пораду або лайфхак на тему цієї категорії та поверни його зі статусом RESPONSE_STATUS: UNIQUE."
    )
...
async def select_and_rewrite_ai_with_gemini(news_list, category_name):
    ...
    prompt = (
        ...
        f"{candidates_text}\n"
        "СУВОРІ ІНСТРУКЦІЇ:\n"
        "1. Обери ОДНОГО найкращого кандидата, який відповідає вказаній категорії.\n"
        "2. Напиши професійний, унікальний та захоплюючий рерайт цієї новини/огляду виключно українською мовою.\n"
        "3. Пост має бути лаконічним і строго до 650-700 символів (разом із пробілами). ВСІ РЕЧЕННЯ МАЮТЬ БУТИ ПОВНІСТЮ ЗАКІНЧЕНИМИ!\n"
        "4. КРИТИЧНО: Заборонено давати будь-які фінансові поради чи торгові сигнали.\n"
        "5. СУВОРЕ ПРАВИЛО: НЕ ПИШИ ЖОДНИХ ВСТУПНИХ СЛІВ, МІРКУВАНЬ, РЕПЛІК ШІ ЧИ ПРИВІТАНЬ ('Давайте проаналізуємо', 'Я проаналізував', 'Ось ідея'). Пиши ОДРАЗУ чистий текст поста.\n"
        "6. Використовуй ТІЛЬКИ HTML-теги для виділення жирного тексту: <b>жирний текст</b>. НІКОЛИ не використовуй маркдаун зі зірочками (типу ** чи *).\n"
        ...
        "8. ВАЖЛИВО: Якщо в обраному пості-кандидаті є готовий промпт або корисний шаблон/код запиту, витягни його та додай наприкінці в теги [PROMPT_START] та [PROMPT_END].\n\n"
        "Почни відповідь з 'INDEX: X' та пиши виключно українською мовою з HTML-форматуванням <b>...</b>."
    )
