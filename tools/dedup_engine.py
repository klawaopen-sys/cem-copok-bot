...
def clean_and_sanitize_final_post(text: str) -> str:
    ...
    # 1. Видаляємо системні мітки та префікси ШІ
    ...
    text = re.sub(r'(?i)^[^\n]*тема\s*створена\s*(ші|штучним\s*інтелектом|ии)[^\n]*\n?', '', text)
    text = re.sub(r'(?i)^[^\n]*тема\s*создана\s*(ии|искусственным\s*интеллектом)[^\n]*\n?', '', text)

    # 2. Видаляємо мета-вступи та міркування ШІ (укр/рос)
    meta_keywords = [
        "тема створена ші", "тема создана ии", "тема створена штучним інтелектом", "тема:",
