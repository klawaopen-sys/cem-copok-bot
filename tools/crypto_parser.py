...
def get_cmc_news():
    ...
    try:
        ...
        if 'data' in data and len(data['data']) > 0:
            # Берем первые 2 заголовка новостей для компактности
            news_items = []
...
async def get_gemini_trader_advice(crypto, forex, market):
    ...
    try:
        ...
        prompt = (
            ...
            "На основі наступних ринкових даних напиши максимально коротку, влучну та практичну пораду для трейдерів на сьогодні "
            "(строго 1 речення, максимум 15 слів, українською мовою). "
            "ВАЖЛИВО: Категорично заборонено згадувати або дублювати у пораді конкретний курс гривні (USD/UAH), курс долара чи інші точні ціни активів, які вже вказані у пості. "
            "Порада має бути лаконічною та давати лише торгову суть без зайвих слів та банальностей:\n"
