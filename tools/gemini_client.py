...
async def _attempt_groq_fallback(json_payload):
    import config
    try:
...
async def _attempt_omnirouter_request(json_payload):
    import config
    try:
        ...
        if omni_key:
            ...
            async with session.post(url, json=payload, headers=headers, timeout=30) as resp:
                if resp.status == 200:
                    ...
                    resp_text = await resp.text()
                    print(f"❌ [Gemini Client] OmniRouter request failed with code {resp.status}: {resp_text}")
    except Exception as e:
        print(f"❌ [Gemini Client] Exception during OmniRouter request: {e}")
    return None

async def gemini_post_with_retry(url, headers, json_payload, timeout=30, retries=3, initial_backoff=2, prefer_groq=False):
    """
    Performs an asynchronous post call to the Gemini API with exponential backoff retries.
    ...
    # Спершу пробуємо OmniRouter, якщо ключ налаштований (як резервний/першочерговий)
    omni_key = getattr(config, 'OMNIROUTER_API_KEY', None)
    
    if prefer_groq:
        print("🌟 [Gemini Client] prefer_groq=True specified. Trying Groq first...")
        groq_res = await _attempt_groq_fallback(json_payload)
    ...
    for attempt in range(1, total_attempts + 1):
        ...
        try:
            ...
            async with session.post(target_url, headers=headers, json=json_payload, timeout=timeout) as r:
                # Оскільки ClientResponse не може бути переданий за межі контекстного менеджера безпосередньо,
                # зчитуємо дані в MockResponse
                status = r.status
                resp_text = await r.text()
                if status == 200:
                    resp_json = await r.json()
                    return MockResponse(resp_json, status)
                else:
                    print(f"⚠️ [Gemini Client] API returned status {status} on attempt {attempt}: {resp_text[:200]}")
                    last_response = MockResponse({"error": resp_text}, status)
        except Exception as e:
            print(f"⚠️ [Gemini Client] Connection error / timeout on attempt {attempt}: {e}")
            last_response = MockResponse({"error": str(e)}, 500)
    ...
    # Резервний каскад: якщо Gemini повернув помилки, пробуємо OmniRouter
    if omni_key:
        print("🚨 [Gemini Client] Gemini failed. Trying OmniRouter as fallback...")
        omni_res = await _attempt_omnirouter_request(json_payload)
        if omni_res is not None and omni_res.status_code == 200:
            return omni_res

    # Наступний резерв: Groq Llama-3.3
    if last_response is None or last_response.status_code != 200:
        print("🚨 [Gemini Client] Gemini and OmniRouter failed. Falling back to Groq...")
        groq_res = await _attempt_groq_fallback(json_payload)
        if groq_res is not None and groq_res.status_code == 200:
            return groq_res
    ...
    if last_response is None:
        try:
            async with session.post(url, headers=headers, json=json_payload, timeout=timeout) as r:
                resp_json = await r.json()
                return MockResponse(resp_json, r.status)
        except Exception as e:
